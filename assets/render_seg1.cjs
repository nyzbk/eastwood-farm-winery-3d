...
async function renderTransition() {
  const startImg = sharp('/home/ubuntu/kontora-site10k/eastwood-farm-winery-3d/assets/keyframes/kf1_mountaintop.jpg').resize(1920, 1080, { fit: 'cover' });
  const endImg = sharp('/home/ubuntu/kontora-site10k/eastwood-farm-winery-3d/assets/keyframes/kf2_the_barn.jpg').resize(1920, 1080, { fit: 'cover' });
  ...
  for (let i = 0; i < 60; i++) {
    ...
    const blended = await sharp(sBufResized)
      ...
      .jpeg({ quality: 92 })
      .toFile(path.join('/home/ubuntu/kontora-site10k/eastwood-farm-winery-3d/assets/temp_seg1', 'frame_' + String(i + 1).padStart(4, '0') + '.jpg'));
  }
}

renderTransition().then(() => console.log('Clip 1 frames synthesized.')).catch(console.error);
